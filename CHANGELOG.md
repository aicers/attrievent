...
- Added `OptionCode` and `OptionData` variants to `DhcpAttr` for DHCP option
  TLV components.
- Added `DceRpcAttr` enum and `DceRpc` variant to `RawEventKind` and
  `RawEventAttrKind` for DCE/RPC protocol support.
